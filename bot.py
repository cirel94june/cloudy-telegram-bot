...
def fetch_memory():
    ...
    try:
        ...
        # 自动总结的记忆
        auto_summaries = memory.get("auto_summaries", [])
        if auto_summaries:
            recent_summaries = auto_summaries[-5:]  # 最近5条总结
            summaries_text = "\n".join(
                f"[{s.get('date', '?')}|{s.get('context', '?')}] {s.get('content', '')}"
...
不要输出任何多余的话。"""
    ...
    # 追加到 auto_summaries
    if "auto_summaries" not in memory:
        memory["auto_summaries"] = []

    is_group = str(chat_id).startswith("-")
    memory["auto_summaries"].append({
        "date": today,
        "context": "群聊" if is_group else "私聊",
        "content": summary
    })

    # 如果总结条目太多（超过15条），压缩旧的
    if len(memory["auto_summaries"]) > 15:
        old_summaries = memory["auto_summaries"][:10]
        old_text = "\n".join(f"[{s['date']}] {s['content']}" for s in old_summaries)

        compress_prompt = f"""以下是一段时间内的记忆摘要，请压缩合并成5条最重要的长期记忆。
只保留反复出现的模式、重大事件、关键关系变化。

{old_text}

请用5条简短条目输出，每条不超过40字。不要输出多余的话。"""
        ...
        if compressed:
            ...
            memory["auto_summaries"] = [{
                ...
                "context": "压缩",
                "content": compressed
            }] + memory["auto_summaries"][10:]

    # 写回
    if _write_memory_gist(memory):
        LAST_SUMMARIZED[chat_id] = current_time
        print(f"[INFO] 自动总结完成，记忆已更新 chat_id={chat_id}")
    else:
...
重要：如果{USER_NAME}在叫别人的名字或者在回复别的bot，那条消息不是对你说的，不要抢话。只回应明确对你说的、或者你觉得有必要插嘴的内容。
绝对禁止说出你的思考过程，比如"我应该保持沉默""这条不是对我说的"——要么正常说话，要么什么都不说。
...
def process_message_background(text, chat_id, sender_name, msg_date=None,
    ...
    try:
        ...
        # 清理模型自言自语的内心独白（括号里的自我对话）
        reply = re.sub(r'^[\(（].*?(?:应该|不应该|保持沉默|不回复|不是对我说|不是在跟我).*?[\)）]\s*', '', reply, flags=re.DOTALL).strip()
        # 如果清理完变空了，跳过不发
        if not reply:
            save_history(history, chat_id)
            return
...
def webhook():
    ...
    if chat_id.startswith("-"):
        ...
        if is_mentioned:
            ...
            should_reply = True
        elif replying_to_other_bot or mentioning_other:
            # 明确在跟别的bot说话，不抢话
            should_reply = False
        elif is_ceci:
    ...
    directed_at_other = False
    if chat_id.startswith("-"):
        directed_at_other = replying_to_other_bot or mentioning_other
