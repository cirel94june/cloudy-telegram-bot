...
ALLOWED_USER_IDS = [i.strip() for i in TG_CHAT_ID_RAW.split(",") if i.strip()]
...
def webhook():
    ...
    # 白名单：允许主人 + 所有bot，屏蔽陌生人
    sender_is_bot = msg.get("from", {}).get("is_bot", False)
    if ALLOWED_USER_IDS and sender_id not in ALLOWED_USER_IDS and not sender_is_bot:
        return "ok"
